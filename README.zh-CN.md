...
直接在终端中渲染**任意 CLI Agent** 输出的 LaTeX 公式，也可以阅读经过排版的
多种文档。
...
下面展示了在 Ghostty 中运行 CLI Agent、介绍麦克斯韦方程时的效果。TFormula
...
把文档路径作为参数传入，即可进入 TFormula 的全屏阅读视图：
...
常见文档扩展名会自动识别。未知文本或二进制文件可用 `--read` 显式打开，避免与
要执行的命令产生歧义：
...
| 格式 | 阅读行为 |
|---|---|
| JSON、JSON Lines、YAML、TOML、XML | 校验并规范化结构化文本；按 `r` 可查看完全不变的原文 |
| CSV、TSV | 固定表头和行号；左右方向键移动可见列窗口 |
| Jupyter Notebook（`.ipynb`） | 在同一文档中显示 Markdown、TeX、代码单元、文本流、错误、HTML 和内嵌图片输出 |
| HTML | 安全解析静态结构、表格、链接、代码和本地图片，绝不执行脚本或样式 |
| EPUB | 将书脊章节、标题、链接、TeX 和书内图片组成一份可搜索文档 |
| PDF | 可搜索的文本重排视图，以及按需生成的原始页面图片视图 |
| ZIP、TAR、TAR.GZ、TGZ、GZIP | 有边界的压缩包目录视图，不任意解压文件 |
| 通过 `--read` 打开的未知二进制 | 文件信息、有限长度的十六进制预览和可打印字符串 |

PDF 支持使用 Poppler。元数据与文本会并行提取；只有按下 `v` 后才会生成原始页面
图片，而且只生成当前选择的一页。macOS 可运行 `brew install poppler`，
Debian/Ubuntu 可运行 `apt install poppler-utils`。没有安装这些工具时，阅读器会
明确显示缺失的后端，不会把 PDF 二进制误当成文本。

普通源码扩展名目前不会被隐式接管，因此 `tformula app.ts` 仍表示执行 `app.ts`。
如确实要阅读源码，可以显式运行 `tformula --read app.ts`，此时按 UTF-8 文本打开。
...
|---|---|
| `j` / `k`、上下方向键 | 上下滚动一行 |
| 左右方向键 | 移动 CSV/TSV/压缩包列窗口；其他文档中左方向键用于返回 |
| `Space` / `b`、Page Down / Up | 上下翻页 |
| `g` / `G` | 跳到开头 / 结尾 |
| `/`、`n` / `N` | 搜索、下一个 / 上一个结果 |
| `t` | 打开文档目录 |
| `l` | 浏览当前文档附近的文件夹和受支持文件 |
| `[` / `]` | 上一个 / 下一个标题；PDF 页面视图中切换页面 |
| `+` / `-` | 放大 / 缩小文档图片 |
| `0` | 恢复图片自动适配 |
| `Tab` / `Shift-Tab`、`Enter` | 选择并打开链接 |
| `h` | 返回上一个本地文档 |
| `r` | 切换渲染视图和原始文本视图 |
| `v` | 切换 PDF 文本重排和原始页面视图 |
| `q` | 退出 |

`l` 文件浏览器不会递归展开可能非常庞大的目录树，而是只按需加载当前一层，并用
压缩面包屑保留清晰的层级位置。文件夹优先显示；上下方向键选择，Enter / 右方向键
进入或打开，左方向键返回上层。目录内容很多时可按 `/` 根据名称即时过滤，界面始终
只渲染终端当前可见的行。
...
- HTML/EPUB 只会被解析为惰性的文档结构，绝不执行脚本和样式；图片只从本地文件
  或 EPUB/Notebook 包内资源加载。
- ZIP/EPUB 展开大小、Notebook 内嵌图片、文档字节数、表格行数和十六进制预览
  都有明确上限；在落盘包内资源之前还会校验压缩包路径。
- PDF 工具通过参数数组直接调用，不经过 Shell；生成的 PDF 页面图片以及解出的
  EPUB/Notebook 临时资源会在阅读器退出时删除。
